...
\frame {
	\frametitle{Initial performance}

	%\includegraphics[width=\textwidth]{cfd_unopt}
}
...
\frame {
	\frametitle{Optimised performance}

	%\includegraphics[width=\textwidth]{cfd_opt}
}
...
\frame {
	...
	%\includegraphics[width=\textwidth]{lft_mpi}
}
...
\frame {
	...
	\begin{itemize}[<+->]
		...
		\item Larger-scale scaling
		\item Combining with automated multithreading
	\end{itemize}
}
...
\frame {
	...
	\begin{itemize}[<+->]
		\item Interested?
		\item Currently recruiting
		\item bit.ly/swansea-rse-2018
		\item Deadline: 20 July (next Friday)
	\end{itemize}
	%\Wider{\includegraphics[width=\textwidth]{swansea-panorama}}
}
